...
public class Vehicle implements Serializable {
    ...
    public void setVIN(String vin) {
        if (vin == null || vin.length() != 17 || !vin.matches("[a-zA-Z0-9]{17}")) {
            throw new IllegalArgumentException("VIN must have 17 alphanumeric characters.");
        }
        this.VIN = vin;
    }
    ...
    public void setBrand(String brand) {
        if (brand == null || !brand.matches("^[a-zA-Z0-9- /]+$")) {
            throw new IllegalArgumentException("Brand name must have only alphanumeric characters.");
        }
        this.brand = brand;
    }
    ...
    public void setModel(String model) {
        if (model == null || !model.matches("^[a-zA-Z0-9- /]+$")) {
            throw new IllegalArgumentException("Model name must have only alphanumeric characters.");
        }
        this.model = model;
    }
    ...
    public void setVehiclePlate(String vehiclePlate) {
        if (vehiclePlate == null || !vehiclePlate.matches("[a-zA-Z0-9]{6}")) {
            throw new IllegalArgumentException("Vehicle plate must have 6 alphanumeric characters.\n");
        }
    ...
    public void setTareWeight(double tareWeight) {
        if (tareWeight <= 0) {
            throw new IllegalArgumentException("Tare weight must be greater than 0 Kg.");
        }
    ...
    public void validateVehicle( LocalDate registrationDate, String vehiclePlate, double tareWeight, double grossWeight,
        ...
        if (year >= 2020) {
            if (!vehiclePlate.matches("^[a-zA-Z]{2}\\d{2}[a-zA-Z]{2}$")) {
                throw new IllegalArgumentException("Vehicle plate must be in the format 'AA00AA'.");
            }
        } else if (year >= 2005) {
            if (!vehiclePlate.matches("^\\d{2}[a-zA-Z]{2}\\d{2}$")) {
                throw new IllegalArgumentException("Vehicle plate must be in the format '00AA00'.");
            }
        } else if (year >= 1992) {
            if (!vehiclePlate.matches("^\\d{4}[a-zA-Z]{2}$")) {
                throw new IllegalArgumentException("Vehicle plate must be in the format '0000AA'.");
            }
        } else if (!vehiclePlate.matches("^[a-zA-Z]{2}\\d{4}$")) {
            throw new IllegalArgumentException("Vehicle plate must be in the format 'AA0000'.");
        }
        if (tareWeight > grossWeight) {
            throw new IllegalArgumentException("Vehicle gross weight must be greater than tare weight.");
        }
