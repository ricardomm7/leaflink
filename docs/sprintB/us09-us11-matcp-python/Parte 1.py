...
dados = pd.read_csv(caminho_arquivo, delimiter=';', decimal=',')

# Input das especificações (Periodo de tempo e nome do Parque)
...
parque_maior_consumo = dados.groupby('Park')['Consumption'].sum().idxmax()
parque_menor_consumo = dados.groupby('Park')['Consumption'].sum().idxmin()

# Filtra os dados para o parque com o maior consumo e menor consumo
dados_parque_maior_consumo = dados[dados['Park'] == parque_maior_consumo]
dados_parque_menor_consumo = dados[dados['Park'] == parque_menor_consumo]
